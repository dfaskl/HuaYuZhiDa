package com.huayu.smartqa.utils;
...
import com.huayu.smartqa.service.TokenCacheService;
...
public class JwtUtils {
    ...
    public String generateToken(String username) {
        ...
        if (user.getDeptTags() != null && !user.getDeptTags().isEmpty()) {
            claims.put("deptTags", user.getDeptTags());
        }
        
        // 添加主组织标签信息
        if (user.getPrimaryDept() != null && !user.getPrimaryDept().isEmpty()) {
            claims.put("primaryDept", user.getPrimaryDept());
        }
     ...
     * 从 JWT Token 中提取组织标签
     */
    public String extractDeptTagsFromToken(String token) {
        try {
            Claims claims = extractClaimsIgnoreExpiration(token);
            return claims != null ? claims.get("deptTags", String.class) : null;
        } catch (Exception e) {
     ...
     */
    public String extractPrimaryDeptFromToken(String token) {
        try {
            Claims claims = extractClaimsIgnoreExpiration(token);
            return claims != null ? claims.get("primaryDept", String.class) : null;
        } catch (Exception e) {
