...
import com.huayu.smartqa.config.KafkaConfig;
import com.huayu.smartqa.exception.CustomException;
import com.huayu.smartqa.model.FileProcessingTask;
import com.huayu.smartqa.model.FileUpload;
import com.huayu.smartqa.model.DepartmentTag;
import com.huayu.smartqa.repository.FileUploadRepository;
import com.huayu.smartqa.service.FileTypeValidationService;
import com.huayu.smartqa.service.ParseService;
import com.huayu.smartqa.service.UploadService;
...
public class UploadController {
     ...
     * @param deptTag 组织标签，如果未指定则使用用户的主组织标签
    ...
    public ResponseEntity<Map<String, Object>> uploadChunk(
            ...
            @RequestParam(value = "deptTag", required = false) String deptTag,
        ...
        try {
            ...
            LogUtils.logBusiness("UPLOAD_CHUNK", userId, "接收到分片上传请求: fileMd5=%s, chunkIndex=%d, fileName=%s, fileType=%s, contentType=%s, fileSize=%d, totalSize=%d, deptTag=%s, isPublic=%s", 
                    fileMd5, chunkIndex, fileName, fileType, contentType, file.getSize(), totalSize, deptTag, isPublic);
            ...
            if (deptTag == null || deptTag.isEmpty()) {
                try {
                    ...
                    deptTag = primaryDept;
                    LogUtils.logBusiness("UPLOAD_CHUNK", userId, "成功获取用户主组织标签: fileName=%s, deptTag=%s", fileName, deptTag);
                } catch (Exception e) {
            ...
            if (!userService.isAdminUser(userId)) {
                DepartmentTag uploadOrg = userService.getDepartmentTag(deptTag);
                ...
                if (exceedsLimit) {
                    ...
                    errorResponse.put("deptTag", deptTag);
            ...
            uploadService.uploadChunk(fileMd5, chunkIndex, totalSize, fileName, file, deptTag, isPublic, userId);
    ...
    public ResponseEntity<Map<String, Object>> mergeFile(
        ...
        try {
            ...
            LogUtils.logBusiness("MERGE_FILE", userId, "创建文件处理任务: fileMd5=%s, fileName=%s, fileType=%s, deptTag=%s, isPublic=%s", 
                    request.fileMd5(), request.fileName(), fileType, fileUpload.getDeptTag(), fileUpload.isPublic());
