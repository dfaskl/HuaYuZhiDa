...
import com.huayu.smartqa.model.RechargePackage;
import com.huayu.smartqa.model.User;
import com.huayu.smartqa.repository.DepartmentTagRepository;
import com.huayu.smartqa.repository.RechargePackageRepository;
import com.huayu.smartqa.repository.UserRepository;
import com.huayu.smartqa.service.ConversationService;
import com.huayu.smartqa.service.InviteCodeService;
import com.huayu.smartqa.service.ModelProviderConfigService;
import com.huayu.smartqa.service.RateLimitConfigService;
...
import com.huayu.smartqa.utils.MinioMigrationUtil;
...
public class AdminController {
    ...
    public ResponseEntity<?> createDepartmentTag(
            @RequestHeader("Authorization") String token,
        ...
        try {
            DepartmentTag tag = userService.createDepartmentTag(
                request.tagId(), 
                request.name(), 
                request.description(), 
                request.parentTag(), 
     ...
     */
    @GetMapping("/org-tags")
    public ResponseEntity<?> getAllDepartmentTags(@RequestHeader("Authorization") String token) {
        String adminUsername = jwtUtils.extractUsernameFromToken(token.replace("Bearer ", ""));
        validateAdmin(adminUsername);
        
        try {
            List<DepartmentTag> tags = DepartmentTagRepository.findAll();
    ...
    public ResponseEntity<?> assignDeptTagsToUser(
            @RequestHeader("Authorization") String token,
        ...
        try {
            userService.assignDeptTagsToUser(userId, request.deptTags(), adminUsername);
    ...
    public ResponseEntity<?> getDepartmentTagTree(
            @RequestHeader("Authorization") String token,
        ...
        try {
            List<Map<String, Object>> tagTree = userService.getDepartmentTagTree();
    ...
    public ResponseEntity<?> updateDepartmentTag(
            @RequestHeader("Authorization") String token,
        ...
        try {
            DepartmentTag updatedTag = userService.updateDepartmentTag(
                tagId, 
                request.name(), 
                request.description(), 
                request.parentTag(), 
    ...
    public ResponseEntity<?> deleteDepartmentTag(
            @RequestHeader("Authorization") String token,
            @PathVariable String tagId) {
        ...
        try {
            userService.deleteDepartmentTag(tagId, adminUsername);
            return ResponseEntity.ok(Map.of(
                "code", 200, 
                "message", "组织标签删除成功"
            ));
    ...
    public ResponseEntity<?> getUserList(
            ...
            @RequestParam(required = false) String deptTag,
        ...
        try {
            Map<String, Object> usersData = userService.getUserList(keyword, deptTag, status, page, size);
            return ResponseEntity.ok(Map.of(
                "code", 200, 
                "message", "获取用户列表成功", 
                "data", usersData
 ...
 * 分配组织标签请求体
 */
record AssignOrgTagsRequest(List<String> deptTags) {}
