...
import com.huayu.smartqa.service.ChatHandler;
import com.huayu.smartqa.service.DocumentService;
...
public class DocumentController {
     ...
     * 
     * @param userId 当前用户ID
     * @param deptTags 用户所属组织标签
     * @return 可访问的文件列表
     */
    @GetMapping("/accessible")
    public ResponseEntity<?> getAccessibleFiles(
            @RequestAttribute("userId") String userId,
            @RequestAttribute("deptTags") String deptTags,
            @RequestParam(required = false) Integer page,
            @RequestParam(required = false) Integer size) {
        
        LogUtils.PerformanceMonitor monitor = LogUtils.startPerformanceMonitor("GET_ACCESSIBLE_FILES");
        try {
            LogUtils.logBusiness("GET_ACCESSIBLE_FILES", userId, "接收到获取可访问文件请求: deptTags=%s", deptTags);
            
            List<FileUpload> files = documentService.getAccessibleFiles(userId, deptTags);
    ...
    private List<Map<String, Object>> convertFilesToResponse(List<FileUpload> files) {
        return files.stream().map(file -> {
            ...
            dto.put("deptTag", file.getDeptTag());
            ...
            dto.put("deptTagName", getDeptTagName(file.getDeptTag()));
            return dto;
    ...
    public ResponseEntity<?> downloadFileByName(
        ...
        try {
            ...
            String deptTags = authContext.deptTags();
            ...
            List<FileUpload> accessibleFiles = documentService.getAccessibleFiles(userId, deptTags);
    ...
    public ResponseEntity<?> previewPdfPage(
        ...
        try {
            ...
            FileUpload file = documentService.getAccessibleFiles(userId, deptTags).stream()
    ...
    public ResponseEntity<?> downloadFileByMd5(
        ...
        try {
            ...
            if (userId == null) {
                ...
                file = documentService.getAccessibleFiles(userId, deptTags).stream()
                        .filter(item -> item.getFileMd5().equals(fileMd5))
                        .findFirst()
                        .orElse(null);
            }
    ...
    public ResponseEntity<?> getReferenceDetail(
        ...
        try {
            ...
            if (authContext.userId() != null) {
                boolean hasAccess = documentService.getAccessibleFiles(authContext.userId(), authContext.deptTags()).stream()
    ...
    private RequestAuthContext resolveRequestAuthContext(String authorization, String fallbackToken) {
        ...
        return new RequestAuthContext(
                ...
                jwtUtils.extractDeptTagsFromToken(jwtToken)
        );
    }
    ...
    private record RequestAuthContext(String userId, String deptTags) {}
     ...
     */
    private String getDeptTagName(String tagId) {
        if (tagId == null || tagId.isEmpty()) {
            return null;
        }
        
        try {
            Optional<DepartmentTag> tagOpt = DepartmentTagRepository.findByTagId(tagId);
            if (tagOpt.isPresent()) {
