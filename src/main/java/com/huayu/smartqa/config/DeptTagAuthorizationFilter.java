 ...
 */
@Component
public class DeptTagAuthorizationFilter extends OncePerRequestFilter {

    private static final Logger logger = LoggerFactory.getLogger(DeptTagAuthorizationFilter.class);
    private static final String DEFAULT_DEPT_TAG = "DEFAULT"; // 默认组织标签
        ...
        try {
            ...
            if (path.matches(".*/upload/chunk.*") ||
                ...
                if (token != null) {
                    ...
                    String deptTags = jwtUtils.extractDeptTagsFromToken(token);
                    if (userId != null) {
                        request.setAttribute("userId", userId);
                        request.setAttribute("role", role);
                        request.setAttribute("deptTags", deptTags);
                        logger.debug("为{}请求设置userId属性: {}, role: {}, deptTags: {}", operation, userId, role, deptTags);
                    } else {
            ...
            String resourceDeptTag = resourceInfo.getDeptTag();
            
            // 如果是公开资源、资源没有组织标签、或属于默认组织，直接放行
            if (resourceInfo.isPublic() || 
                resourceDeptTag == null || 
                resourceDeptTag.isEmpty() || 
                DEFAULT_DEPT_TAG.equals(resourceDeptTag)) {
            ...
            // 检查是否为私人组织标签资源
            if (resourceDeptTag.startsWith(PRIVATE_TAG_PREFIX)) {
            ...
            // 获取用户的组织标签
            String userDeptTags = jwtUtils.extractDeptTagsFromToken(token);
            if (userDeptTags == null || userDeptTags.isEmpty()) {
            ...
            // 检查用户是否有权限访问该资源
            if (isUserAuthorized(userDeptTags, resourceDeptTag)) {
    ...
    private ResourceInfo getResourceInfo(String resourceId) {
        ...
        if (fileUpload.isPresent()) {
            ...
            logger.debug("成功找到文件资源信息 => 资源ID: {}, 拥有者: {}, 组织标签: {}, 是否公开: {}", 
                        resourceId, info.getOwner(), info.getDeptTag(), info.isPublic());
            return info;
        } else {
     ...
     * 检查用户是否有权限访问该资源
     */
    private boolean isUserAuthorized(String userDeptTags, String resourceDeptTag) {
        // 将用户的组织标签字符串转换为集合
        Set<String> userTags = Arrays.stream(userDeptTags.split(","))
                .collect(Collectors.toSet());
        
        // 检查用户的组织标签是否包含资源的组织标签
        return userTags.contains(resourceDeptTag);
    }
    ...
    private static class ResourceInfo {
        ...
        private final String deptTag;
        private final boolean isPublic;
        
        public ResourceInfo(String owner, String deptTag, boolean isPublic) {
            this.owner = owner;
