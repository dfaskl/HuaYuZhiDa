...
import com.huayu.smartqa.client.EmbeddingClient;
import com.huayu.smartqa.entity.EsDocument;
import com.huayu.smartqa.entity.SearchResult;
...
public class HybridSearchService {
    ...
    public List<SearchResult> searchWithPermission(String query, String userId, int topK) {
        ...
        try {
            // 获取用户有效的组织标签（包含层级关系）
            List<String> userEffectiveTags = getUserEffectiveDeptTags(userId);
                                ...
                                .filter(f -> f.bool(bf -> bf
                                        ...
                                        .should(s3 -> {
                                            if (userEffectiveTags.isEmpty()) {
                                                ...
                                                return s3.term(t -> t.field("deptTag").value(userEffectiveTags.get(0)));
                                            } else {
                                                return s3.bool(inner -> {
                                                    userEffectiveTags.forEach(tag -> inner.should(sh2 -> sh2.term(t -> t.field("deptTag").value(tag))));
                                                    return inner;
                                                });
                                            }
                                        })
            ...
            try {
                ...
                return textOnlySearchWithPermission(query, getUserDbId(userId), getUserEffectiveDeptTags(userId), topK);
     ...
     * 获取用户的有效组织标签（包含层级关系）
     */
    private List<String> getUserEffectiveDeptTags(String userId) {
        ...
        try {
            ...
            // 通过DeptTagCacheService获取用户的有效标签集合
            List<String> effectiveTags = DeptTagCacheService.getUserEffectiveDeptTags(user.getUsername());
