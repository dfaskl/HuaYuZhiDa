...
import com.huayu.smartqa.config.AppAuthProperties;
import com.huayu.smartqa.exception.CustomException;
import com.huayu.smartqa.model.RegistrationMode;
import com.huayu.smartqa.model.DepartmentTag;
import com.huayu.smartqa.model.User;
import com.huayu.smartqa.repository.DepartmentTagRepository;
import com.huayu.smartqa.repository.UserRepository;
import com.huayu.smartqa.utils.PasswordUtil;
...
public class UserService {
    ...
    @Autowired
    private DeptTagCacheService DeptTagCacheService;
    ...
    public void registerUser(String username, String password, String inviteCode) {
        ...
        List<String> assignedOrgTags = List.of(DEFAULT_DEPT_TAG, privateTagId);
        user.setDeptTags(String.join(",", assignedOrgTags));
        
        // 设置私人组织标签为主组织标签
        user.setPrimaryDept(privateTagId);
        
        userRepository.save(user);
        
        // 缓存组织标签信息
        DeptTagCacheService.cacheUserDeptTags(username, assignedOrgTags);
        DeptTagCacheService.cacheUserPrimaryDept(username, privateTagId);
    ...
    private void createPrivateOrgTag(String privateTagId, String username, User owner) {
        // 检查私人标签是否已存在
        if (!DepartmentTagRepository.existsByTagId(privateTagId)) {
            logger.info("Creating private organization tag for user: {}", username);
            
            // 创建私人组织标签
            DepartmentTag privateTag = new DepartmentTag();
            ...
            DepartmentTagRepository.save(privateTag);
    ...
    private void ensureDefaultOrgTagExists() {
        if (!DepartmentTagRepository.existsByTagId(DEFAULT_DEPT_TAG)) {
            ...
            // 创建默认组织标签
            DepartmentTag defaultTag = new DepartmentTag();
            defaultTag.setTagId(DEFAULT_DEPT_TAG);
            defaultTag.setName(DEFAULT_ORG_NAME);
            defaultTag.setDescription(DEFAULT_ORG_DESCRIPTION);
            defaultTag.setCreatedBy(creator);
            
            DepartmentTagRepository.save(defaultTag);
     ...
     */
    @Transactional
    public DepartmentTag createDepartmentTag(String tagId, String name, String description, 
        ...
        if (parentTag != null && !parentTag.isEmpty()) {
            DepartmentTagRepository.findByTagId(parentTag)
                    .orElseThrow(() -> new CustomException("Parent tag not found", HttpStatus.NOT_FOUND));
        }
        
        DepartmentTag tag = new DepartmentTag();
        ...
        DepartmentTag savedTag = DepartmentTagRepository.save(tag);
        
        // 清除标签缓存，因为层级关系可能变化
        DeptTagCacheService.invalidateAllEffectiveTagsCache();
     ...
     * 
     * @param userId 用户ID
     * @param deptTags 组织标签ID列表
     * @param adminUsername 管理员用户名
     */
    @Transactional
    public void assignDeptTagsToUser(Long userId, List<String> deptTags, String adminUsername) {
        // 验证操作者是否为管理员
        ...
        // 验证所有标签是否存在
        for (String tagId : deptTags) {
            if (!DepartmentTagRepository.existsByTagId(tagId)) {
        ...
        if (user.getDeptTags() != null && !user.getDeptTags().isEmpty()) {
            existingTags = Arrays.stream(user.getDeptTags().split(",")).collect(Collectors.toSet());
        }
        ...
        // 确保用户的私人组织标签不会被删除
        Set<String> finalTags = new HashSet<>(deptTags);
        if (hasPrivateTag && !finalTags.contains(privateTagId)) {
            finalTags.add(privateTagId);
        }
        
        // 将标签列表转换为逗号分隔的字符串
        String deptTagsStr = String.join(",", finalTags);
        user.setDeptTags(deptTagsStr);

        // 如果用户没有主院系标签且有院系标签，则优先使用私人标签作为主院系
        if ((user.getPrimaryDept() == null || user.getPrimaryDept().isEmpty()) && !finalTags.isEmpty()) {
            if (hasPrivateTag) {
                user.setPrimaryDept(privateTagId);
            } else {
                user.setPrimaryDept(new ArrayList<>(finalTags).get(0));
            }
        }
        ...
        // 更新缓存
        DeptTagCacheService.deleteUserDeptTagsCache(user.getUsername());
        DeptTagCacheService.cacheUserDeptTags(user.getUsername(), new ArrayList<>(finalTags));
        // 同时清除有效标签缓存
        DeptTagCacheService.deleteUserEffectiveTagsCache(user.getUsername());
        
        if (user.getPrimaryDept() != null && !user.getPrimaryDept().isEmpty()) {
            DeptTagCacheService.cacheUserPrimaryDept(user.getUsername(), user.getPrimaryDept());
        }
    }
     ...
     */
    public Map<String, Object> getUserDeptTags(String username) {
        User user = userRepository.findByUsername(username)
                .orElseThrow(() -> new CustomException("User not found", HttpStatus.NOT_FOUND));
        
        // 尝试从缓存获取
        List<String> deptTags = DeptTagCacheService.getUserDeptTags(username);
        String primaryDept = DeptTagCacheService.getUserPrimaryDept(username);
        
        // 如果缓存中没有，则从数据库获取
        if (deptTags == null || deptTags.isEmpty()) {
            deptTags = Arrays.asList(user.getDeptTags().split(","));
            // 更新缓存
            DeptTagCacheService.cacheUserDeptTags(username, deptTags);
        }
        
        if (primaryDept == null || primaryDept.isEmpty()) {
            primaryDept = user.getPrimaryDept();
            // 更新缓存
            DeptTagCacheService.cacheUserPrimaryDept(username, primaryDept);
        }
        
        // 获取组织标签的详细信息
        List<Map<String, Object>> deptTagDetails = new ArrayList<>();
        for (String tagId : deptTags) {
            DepartmentTag tag = DepartmentTagRepository.findByTagId(tagId)
                    .orElse(null);
            if (tag != null) {
                ...
                deptTagDetails.add(tagInfo);
            }
        }
        
        Map<String, Object> result = new HashMap<>();
        result.put("deptTags", deptTags);
        result.put("primaryDept", primaryDept);
        result.put("deptTagDetails", deptTagDetails);
     ...
     */
    public void setUserPrimaryDept(String username, String primaryDept) {
        User user = userRepository.findByUsername(username)
                .orElseThrow(() -> new CustomException("User not found", HttpStatus.NOT_FOUND));
        
        // 检查该组织标签是否已分配给用户
        Set<String> userTags = Arrays.stream(user.getDeptTags().split(",")).collect(Collectors.toSet());
        if (!userTags.contains(primaryDept)) {
            throw new CustomException("Organization tag not assigned to user", HttpStatus.BAD_REQUEST);
        }
        
        user.setPrimaryDept(primaryDept);
        userRepository.save(user);
     ...
     * @return 用户的主组织标签
     */
    public String getUserPrimaryDept(String userId) {
        ...
        if (primaryDept == null || primaryDept.isEmpty()) {
            ...
            if (primaryDept == null || primaryDept.isEmpty()) {
                String[] tags = user.getDeptTags().split(",");
                if (tags.length > 0) {
                    primaryDept = tags[0];
                    // 更新用户的主组织标签
                    user.setPrimaryDept(primaryDept);
     ...
     * @return 组织标签树结构
     */
    public List<Map<String, Object>> getDepartmentTagTree() {
        // 获取所有根节点（parentTag为null的标签）
        List<DepartmentTag> rootTags = DepartmentTagRepository.findByParentTag(null);
     ...
     * @return 树形结构
     */
    private List<Map<String, Object>> buildTagTreeRecursive(List<DepartmentTag> tags) {
        List<Map<String, Object>> result = new ArrayList<>();
        
        for (DepartmentTag tag : tags) {
            ...
            // 获取子标签
            List<DepartmentTag> children = DepartmentTagRepository.findByParentTag(tag.getTagId());
            if (!children.isEmpty()) {
     ...
     */
    @Transactional
    public DepartmentTag updateDepartmentTag(String tagId, String name, String description, 
        ...
        DepartmentTag updatedTag = DepartmentTagRepository.save(tag);
    ...
    public DepartmentTag getDepartmentTag(String tagId) {
        return DepartmentTagRepository.findByTagId(tagId)
                .orElseThrow(() -> new CustomException("Organization tag not found", HttpStatus.NOT_FOUND));
    }
    ...
    private boolean wouldFormCycle(String tagId, String newParentId) {
        ...
        while (currentParentId != null && !currentParentId.isEmpty()) {
            ...
            // 获取父标签的父标签
            Optional<DepartmentTag> parentTag = DepartmentTagRepository.findByTagId(currentParentId);
     ...
     */
    @Transactional
    public void deleteDepartmentTag(String tagId, String adminUsername) {
        // 验证操作者是否为管理员
        User admin = userRepository.findByUsername(adminUsername)
                .orElseThrow(() -> new CustomException("Admin not found", HttpStatus.NOT_FOUND));
        ...
        // 检查是否是特殊标签（如默认标签）
        if (DEFAULT_DEPT_TAG.equals(tagId)) {
            throw new CustomException("Cannot delete the default organization tag", HttpStatus.BAD_REQUEST);
        }
        
        // 检查是否有子标签
        List<DepartmentTag> children = DepartmentTagRepository.findByParentTag(tagId);
        if (!children.isEmpty()) {
        ...
        // 删除标签
        DepartmentTagRepository.delete(tag);
     ...
     * 
     * @param keyword 搜索关键词
     * @param deptTag 组织标签过滤
     * @param status 用户状态过滤
     * @param page 页码
     * @param size 每页大小
     * @return 用户列表数据
     */
    public Map<String, Object> getUserList(String keyword, String deptTag, Integer status, int page, int size) {
        ...
        List<User> filteredUsers = userRepository.findAll(Sort.by("createdAt").descending()).stream()
                .filter(user -> matchesUserListFilters(user, keyword, deptTag, status))
                .toList();
        ...
        List<Map<String, Object>> userList = userPage.getContent().stream()
                .map(user -> {
                    ...
                    // 获取用户组织标签的详细信息
                    List<Map<String, String>> deptTagDetails = new ArrayList<>();
                    if (user.getDeptTags() != null && !user.getDeptTags().isEmpty()) {
                        Arrays.stream(user.getDeptTags().split(","))
                                .forEach(tagId -> {
                                    DepartmentTag tag = DepartmentTagRepository.findByTagId(tagId)
                                            .orElse(null);
                                    if (tag != null) {
                    ...
                    userMap.put("deptTags", deptTagDetails);
                    userMap.put("primaryDept", user.getPrimaryDept());
    ...
    private boolean matchesUserListFilters(User user, String keyword, String deptTag, Integer status) {
        if (deptTag != null && !deptTag.isEmpty()) {
            if (user.getDeptTags() == null || user.getDeptTags().isEmpty()) {
                return false;
            }

            Set<String> userTags = new HashSet<>(Arrays.asList(user.getDeptTags().split(",")));
            if (!userTags.contains(deptTag)) {
                return false;
            }
        }
    ...
    private String resolveOrGenerateTagId(String tagId, String name) {
        ...
        if (!normalizedTagId.isEmpty()) {
            ...
            if (DepartmentTagRepository.existsByTagId(normalizedTagId)) {
    ...
    private String generateUniqueTagId(String name) {
        ...
        if (!DepartmentTagRepository.existsByTagId(baseId)) {
            return baseId;
        }

        for (int i = 2; i <= 9999; i++) {
            String candidate = appendSuffix(baseId, "_" + i);
            if (!DepartmentTagRepository.existsByTagId(candidate)) {
                return candidate;
            }
        }
