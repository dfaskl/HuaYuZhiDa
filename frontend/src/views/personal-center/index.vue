...
const tags = ref<Api.DeptTag.Mine>({
  deptTags: [],
  primaryDept: '',
  deptTagDetails: []
});
...
const getDeptTags = async () => {
  const { error, data } = await request<Api.DeptTag.Mine>({
    url: '/users/dept-tags'
  });
  if (!error) {
    tags.value = data;
...
const showModal = (tagId: string) => {
  if (tagId === tags.value.primaryDept) return;
  visible.value = true;
...
const setPrimaryOrg = async () => {
  ...
  const { error } = await request({
    ...
    method: 'PUT',
    data: { primaryDept: currentTagId.value, userId: userInfo.value.id }
  });
  if (!error) {
    visible.value = false;
    getDeptTags();
      ...
      <NCard class="min-h-400px min-w-600px w-70vw card-wrapper" :segmented="{ content: true, footer: 'soft' }">
        ...
        <NScrollbar class="max-h-60vh">
          <div class="flex flex-col gap-4 p-4">
            ...
            <NCard
              v-for="tag in tags.deptTagDetails"
              :key="tag.tagId"
              size="small"
              embedded
              hoverable
            ...
            >
              <div class="flex items-center justify-between">
                <div>{{ tag.name }}</div>
                <NTag v-if="tag.tagId === tags.primaryDept" type="primary" size="small">
                  主标签
                  <template #icon>
