...
import { fetchGetDeptTagList } from '@/service/api/dept-tag';
import DeptTagOperateDialog from './modules/dept-tag-operate-dialog.vue';

const appStore = useAppStore();

const { columns, columnChecks, data, loading, getData, mobilePagination } = useTable({
  apiFn: fetchGetDeptTagList,
  showTotal: true,
  columns: () => [
    {
      key: 'name',
...
const {
  ...
  onDeleted
  // closeDrawer
} = useTableOperate<Api.DeptTag.Item>(getData);

function addChild(row: Api.DeptTag.Item) {
  handleAddChild(row);
}

/** the editing row data */
function edit(row: Api.DeptTag.Item) {
  handleEdit(row);
}
...
<template>
  <div class="flex-col-stretch gap-16px overflow-hidden <sm:overflow-auto">
    <NCard title="院系" :bordered="false" size="small" class="sm:flex-1-hidden card-wrapper">
