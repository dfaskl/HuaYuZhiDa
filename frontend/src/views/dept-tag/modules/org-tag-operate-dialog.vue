...
defineOptions({
  name: 'DeptTagOperateDialog'
});

const props = defineProps<{
  operateType: NaiveUI.TableOperateType;
  rowData: Api.DeptTag.Item;
  data: Api.DeptTag.Item[];
}>();
...
const model = ref<Api.DeptTag.Item>(createDefaultModel());

function createDefaultModel(): Api.DeptTag.Item {
  return {
    tagId: '',
    name: '',
    description: '',
  ...
  >
    <NForm ref="formRef" :model="model" :rules="rules" label-placement="left" :label-width="100" mt-10>
      ...
      <NFormItem label="所属标签" path="parentTag">
        <DeptTagCascader v-model:value="model.parentTag" :options="data" />
      </NFormItem>
