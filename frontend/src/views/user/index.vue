...
import DeptTagSettingDialog from './modules/dept-tag-setting-dialog.vue';
    ...
    },
    {
      ...
      render: row => (
        <div class="flex flex-wrap gap-2">
          {row.deptTags.map(tag => (
            <NTag key={tag.tagId} type={tag.tagId === row.primaryDept ? 'primary' : 'default'}>
              {tag.name}
            </NTag>
          ))}
        </div>
    ...
    },
    {
      ...
      render: row => (
        <div class="flex gap-2">
          <NButton type="primary" ghost size="small" onClick={() => handleDeptTag(row)}>
            分配院系
          </NButton>
          {authStore.isAdmin ? (
...
function handleDeptTag(row: Api.User.Item) {
...
<template>
  <div class="min-h-500px flex-col-stretch gap-16px overflow-auto">
    ...
    <DeptTagSettingDialog v-model:visible="visible" :row-data="editingData!" @submitted="getData" />
